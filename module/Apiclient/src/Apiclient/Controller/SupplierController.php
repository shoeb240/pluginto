...
class SupplierController extends ApiclientActionController
{
    ...
    public function addAction()
    {
        ...
        if ($this->request->isPost()) {
            $createData = array('supplier_name' => $this->getRequest()->getPost('supplier_name'),
            ...
            if ($response->getStatusCode() == 200 && $result->data) {
                return $this->redirect()->toRoute(null, array('controller' => 'supplier', 'action'=>'index'));
            } else {
                $params['error_code'] = $result->error_code;
                $params['error_msg'] = $result->error_msg;
            }
    ...
    public function editAction()
    {
        ...
        if ($this->request->isPost()) {
            $updatedData = array('supplier_name' => $this->getRequest()->getPost('supplier_name'),
                                'display_name' => $this->getRequest()->getPost('display_name'),
                                'name' => $this->getRequest()->getPost('name'),
                                'surname' => $this->getRequest()->getPost('surname'),
                                'address1' => $this->getRequest()->getPost('address1'),
        ...
        $apiController = $this->prepareApi($request);
        $params['supplier'] = $apiController->dispatch($request)->data;
    ...
    private function prepareApi(Request $request)
    {
        ...
        $routeMatch       = new \Zend\Mvc\Router\RouteMatch(array('controller' => 'input', 'input-type' => 'supplier'));
