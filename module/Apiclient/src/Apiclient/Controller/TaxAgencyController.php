...
class TaxAgencyController extends ApiclientActionController
{
    ...
    public function addAction()
    {
        ...
        if ($this->request->isPost()) {
            $createData = array('display_name' => $this->getRequest()->getPost('display_name'));
            ...
            if ($response->getStatusCode() == 200 && $result->data) {
                return $this->redirect()->toRoute(null, array('controller' => 'tax-agency', 'action'=>'index'));
            } else {
                $params['error_code'] = $result->error_code;
                $params['error_msg'] = $result->error_msg;
            }
    ...
    public function editAction()
    {
        ...
        if ($this->request->isPost()) {
            $updatedData = array('company_name' => $this->getRequest()->getPost('company_name'),
        ...
        $apiController = $this->prepareApi($request);
        $params['company'] = $apiController->dispatch($request)->data;
    ...
    private function prepareApi(Request $request)
    {
        ...
        $routeMatch       = new \Zend\Mvc\Router\RouteMatch(array('controller' => 'input', 'input-type' => 'TaxAgency'));
